require([
  ...
  search.on("select-result", function (event) {
    ...
    view.popup.open({
      ...
      content: address + "<br><br> <button type='button' class='bg-gray-800 text-sm text-gray-100 py-1 px-4 mx-1 my-2 rounded-lg shadow-md' onclick='addStop(mapPoint); view.popup.close()'><i class='fas fa-map-marker-alt mr-2'></i>Agregar Parada</button>",
    });
  })
}
)
